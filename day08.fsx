...
let getOutputValue (digits : string array) =
    ...
    let five    = d |> Seq.except [three] |> Seq.where (Seq.length >> (=) 5) |> Seq.where (Set.isSuperset nine) |> Seq.head
    let two     = d |> Seq.except [five;three] |> Seq.where (Seq.length >> (=) 5) |>  Seq.head
    let six     = d |> Seq.except [nine] |> Seq.where (Seq.length >> (=) 6) |> Seq.where (Set.isSubset five) |> Seq.head
